...
def get_post_by_id(id: str, request: Request):
...
def like_post(
    ...
    status_name, post = db_handler.like_post(id, current_user["id"])
...
def unlike_post(
	...
	status_name, post = db_handler.unlike_post(id, current_user["id"])
	if status_name == "missing":
		raise PostNotFound(id)
